...
def delete_pcluster(cluster_name: str, region: str, dry_run: bool, session: boto3.Session) -> bool:
    """Delete the ParallelCluster CloudFormation stack."""
    path = shutil.which("pcluster")
    status = get_pcluster_status(cluster_name, session)
    
    if status == "NOT_FOUND":
        console.print(f"[yellow]⚠ Cluster '{cluster_name}' not found in CloudFormation. Skipping delete.[/yellow]")
        return True

    console.print(Panel(f"[bold]Deleting ParallelCluster: {cluster_name}[/bold]", border_style="red"))
    ...
    # 1. Trigger Deletion
    console.print(f"[cyan]→ pcluster delete-cluster --cluster-name {cluster_name} ...[/cyan]")
    cmd = [
    ...
    with Progress(SpinnerColumn(), TextColumn("{task.description}"), console=console) as progress:
        ...
        while True:
            ...
            if 'DELETE_FAILED' in status:
                progress.update(task, description=f"[red]❌ Deletion failed: {status}[/red]")
                return False
...
def main():
    ...
    if not region:
        console.print("[red]❌ No region specified. Use --region or configured profile.[/red]")
        sys.exit(1)
    ...
    if not args.force:
        ...
        if tofu_match:
            console.print(f"  2. Associated Connectivity & Event Infrastructure (Tofu resources)")
            console.print(f"     Target Cluster ID: {default_id}")
        else:
            console.print(f"  [dim](Skipping Tofu destroy: active state is for '{default_name}', not '{cluster_name}')[/dim]")
        ...
        confirm_id = questionary.text(
            ...
            validate=lambda val: len(val) == 8 and val.startswith('clus') or "Must be valid ID (clus...)"
        ).ask()
        
        if confirm_id is None: sys.exit(0)
        
        # If we are potentially destroying Tofu, ID *must* match tfvars
        if tofu_match and confirm_id != default_id:
            console.print(f"[red]❌ Mismatch! You entered '{confirm_id}' but active Tofu state is '{default_id}'.[/red]")
            console.print("[red]Aborting to prevent accidental deletion of wrong resources.[/red]")
            sys.exit(1)
    ...
    if default_name == cluster_name:
        ...
        console.print(f"[yellow]⚠ Preserving Tofu state (belongs to '{default_name}')[/yellow]")
